...
def gen_ip4_data() -> bytes:
    ...
    with open('./data/ipv4.txt', 'wb') as file:
        file.write(geo)
    with open('./data/ipv4le.bin', 'wb') as file:
        file.write(b''.join(pack('<', int(ip)) for ip in iplist))
    with open('./data/ipv4be.bin', 'wb') as file:
        file.write(b''.join(pack('>', int(ip)) for ip in iplist))
...
def gen_ip6_data() -> bytes:
    ...
    with open('./data/ipv6.txt', 'wb') as file:
        file.write(geo)
    with gzip.GzipFile('./data/ipv6le.gz', 'wb', mtime=1) as file:
        file.write(b''.join(pack('<', int(ip)) for ip in iplist))
    with gzip.GzipFile('./data/ipv6be.gz', 'wb', mtime=1) as file:
        file.write(b''.join(pack('>', int(ip)) for ip in iplist))
